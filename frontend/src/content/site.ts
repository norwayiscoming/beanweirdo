...
/**
 * Sidebar section headings, and the first step of every breadcrumb.
 *
 * These used to be three editable fields, on the strength of "the site owner
 * might want to rename them". Nobody ever did, and the three boxes they needed
 * on the CMS cost more attention than the rename would have saved — so they
 * are code now, like every other word the back office does not offer to
 * change.
 */
export const SECTION_NAMES: Record<NavGroup, string> = {
  Public: 'Public',
  Practice: 'Practice',
  Admin: 'Admin',
}
...
export type SiteOverrides = Partial<SiteCopy> & {
  /**
